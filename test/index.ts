...
Hoek.merge(target, null);           // results in { a: 1, b: 2 }
...
Hoek.deepEqual(null, undefined); //results in false
...
const newArray3 = Hoek.intersect(new Set(array), new Set(array2)); // results in [1]
...
const sym = Symbol('d');
const obj1 = { a: { b: { c: 1 }, [sym]: { e : 2 } } };

Hoek.reach(obj1, 'a.b.c'); // returns 1
Hoek.reach(obj1, ['a', sym, 'e']); // returns 2

const obj2 = { a: { b: [2, 3, 6] } };

Hoek.reach(obj2, 'a.b.-1'); // returns 6
...
Hoek.wait(10).then(Hoek.ignore);

// block

Hoek.block().then(Hoek.ignore);
