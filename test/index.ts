...
import { type MergeTypes } from '../src/merge.js';
...
describe('merge()', () => {
    ...
    it('overrides values in target', () => {
        ...
        expect(c.z).toEqual(4);
        expect(c.v).toEqual(0);
        expect(c.m).toEqual('123');
        expect(c.t).toHoequal({ u: 6 });
        expect(c.s).toBeUndefined();
    });
    ...
    it('overrides values in target (flip)', () => {
        ...
        expect(d.y).toEqual(2);
        expect(d.z).toEqual(3);
        expect(d.v).toEqual(5);
        expect(d.m).toEqual('abc');
        expect(d.t).toEqual('test');
        expect(d.s).toEqual(1);
    });
    ...
    it('overrides Buffer', () => {
        ...
        Hoek.merge({ x: {} }, a);
        expect(a.x.toString()).toEqual('abc');
    });
...
describe('intersect()', () => {
    ...
    it('returns an empty array if either input is null', () => {
        expect(Hoek.intersect([1], null)).toHaveLength(0);
        expect(Hoek.intersect(null, [1])).toHaveLength(0);
...
describe('contain()', () => {
    ...
    it('tests objects', () => {
...

        {
            ...
            expect(new Bar().a).toEqual(1);
            expect(new Bar().b).toEqual(2);
            expect(new Bar().c).toEqual(3);
...
describe('flatten()', () => {
    ...
    it('appends to a target array', () => {
        const target = [4, [5]];
        const result = Hoek.flatten([1, [2, 3]], target);

        expect(result).toBe(target); // Target is modified in place
        expect(result).toHoequal([4, [5], 1, 2, 3]); // Target items are not flattened
    });

    it('keeps array holes', () => {
        // oxlint-disable-next-line no-sparse-arrays -- the hole is the point of the test
        expect(Hoek.flatten([1, , 2])).toHoequal([1, undefined, 2]);
    });
});
...
describe('reach()', () => {
    ...
    it('returns values of array', () => {
        expect(Hoek.reach(obj, 'k.0')).toEqual(4);
        expect(Hoek.reach(obj, 'k.1')).toEqual(8);
    });

    it('returns last value of array using negative index', () => {
        expect(Hoek.reach(obj, 'k.-1')).toEqual(1);
        expect(Hoek.reach(obj, 'k.-2')).toEqual(9);
    });
    ...
    it('returns a valid member', () => {
        expect(Hoek.reach(obj, 'a.b.c.d')).toEqual(1);
    });
    ...
    it('returns undefined on null object', () => {
        expect(Hoek.reach(null, 'a.b.c.d')).toBeUndefined();
    });

    it('returns undefined on missing object member', () => {
        expect(Hoek.reach(obj, 'a.b.c.d.x')).toBeUndefined();
    });

    it('returns undefined on missing function member', () => {
        expect(Hoek.reach(obj, 'i.y', { functions: true })).toBeUndefined();
    });
    ...
    it('returns undefined on invalid member', () => {
        expect(Hoek.reach(obj, 'a.b.c.d-.x')).toBeUndefined();
        expect(Hoek.reach(obj, 'k.x')).toBeUndefined();
        expect(Hoek.reach(obj, 'k.1000')).toBeUndefined();
        expect(Hoek.reach(obj, 'k/0.5', '/')).toBeUndefined();
    });

    it('returns function member', () => {
        expect(typeof Hoek.reach(obj, 'i')).toEqual('function');
    });

    it('returns function property', () => {
        expect(Hoek.reach(obj, 'i.x')).toEqual(5);
    });

    it('returns null', () => {
        expect(Hoek.reach(obj, 'j')).toBeNull();
    });
    ...
    it('will return a default value if property is not found', () => {
        expect(Hoek.reach(obj, 'a.b.q', { default: 'defaultValue' })).toEqual('defaultValue');
    });

    it('will return a default value if path is not found', () => {
        expect(Hoek.reach(obj, 'q', { default: 'defaultValue' })).toEqual('defaultValue');
    });

    it('allows a falsey value to be used as the default value', () => {
        expect(Hoek.reach(obj, 'q', { default: '' })).toEqual('');
    });
    ...
    it('allows array-based lookup with symbols', () => {
        ...
        expect(Hoek.reach(obj, ['a', Symbol(), 'v'])).toBeUndefined();
    });

    it('returns character in string', () => {
        expect(Hoek.reach(['abc'], [0])).toEqual('abc');
        expect(Hoek.reach(['abc'], ['0'])).toEqual('abc');
    });
    ...
    it('reaches sets and maps', () => {
        ...
        expect(Hoek.reach(value, 'a.b.2.y.w', { iterables: true })).toEqual(5);
    });

    it('reaches set and map properties when iterables is not enabled', () => {
        const set = new Set([1, 2]) as Set<number> & { custom: string };
        set.custom = 'x';

        expect(Hoek.reach(set, 'size')).toEqual(2);
        expect(Hoek.reach(set, 'custom')).toEqual('x');
        expect(Hoek.reach(new Map([['a', 1]]), 'size')).toEqual(1);
    });

    it('throws on a set or map segment when strict is enabled', () => {
        const value = { a: new Map([['b', { c: 1 }]]) };

        expect(() => Hoek.reach(value, 'a.b.c', { strict: true })).toThrow('Missing segment b in reach path  a.b.c');
        expect(Hoek.reach(value, 'a.b.c')).toBeUndefined();
        expect(Hoek.reach(value, 'a.b.c', { iterables: true })).toEqual(1);
    });
});
...
describe('reachTemplate()', () => {
    it('applies object to template', () => {
        ...
        expect(Hoek.reachTemplate(obj, template)).toEqual('4:9:1::');
    });
...
describe('AssertError', () => {
    it('takes an optional message', () => {
        expect(new Hoek.AssertError().message).toEqual('Unknown error');
        expect(new Hoek.AssertError(null).message).toEqual('Unknown error');
        expect(new Hoek.AssertError('msg').message).toEqual('msg');
    });

    it('has AssertError name property', () => {
        expect(new Hoek.AssertError().name).toEqual('AssertError');
        expect(new Hoek.AssertError('msg').name).toEqual('AssertError');
    });
...
describe('escapeRegex()', () => {
    it('escapes all special regular expression characters', () => {
        ...
        expect(a).toEqual('4\\^f\\$s\\.4\\*5\\+\\-_\\?%\\=#\\!\\:@\\|~\\\\\\/`"\\(>\\)\\[<\\]d\\{\\}s\\,');
    });
});
...
describe('ignore()', () => {
    it('exists', () => {
        ...
        expect(typeof Hoek.ignore).toEqual('function');
    });
});
...
describe('stringify()', () => {
    it('converts object to string', () => {
        const obj = { a: 1 };
        expect(Hoek.stringify(obj)).toEqual('{"a":1}');
    });
