...
export const reach = <T>(
    ...
    for (let i = 0; i < path.length; ++i) {
        let key = path[i];
        // Defaults to false, in which case Set and Map are traversed as plain objects

        const isSetType = !!options.iterables && isSet(ref);
        const isMapType = !!options.iterables && isMap(ref);
