...
import type { ShallowKeys } from './clone.js';
...
const applyToDefaultsWithShallow = function <T extends object, U extends object>(
    ...
    const seen = new Map();
    const sourcePathsToShallowCopy = source === true ? null : new Set();
    ...
    for (const k of keys) {
        ...
        if (ref && typeof ref === 'object') {
            seen.set(ref, (sourcePathsToShallowCopy && reach(source as U, key as string[])) || ref);
        } else if (sourcePathsToShallowCopy) {
            sourcePathsToShallowCopy.add(key);
        }
    }

    const copy = clone(defaults, {}, seen);

    if (!sourcePathsToShallowCopy) {
        return copy;
    }

    for (const key of sourcePathsToShallowCopy) {
