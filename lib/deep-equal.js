...
internals.getInternalType = function (obj) {
    ...
    if (Array.isArray(obj)) {
        ...
        return internals.setType;
    }
    else if (obj instanceof Map) {
        return internals.mapType;
    }
    else if (obj instanceof WeakSet) {
        return internals.weakSetType;
    }
    else if (obj instanceof WeakMap) {
        return internals.weakMapType;
    }
...
internals.isDeepEqualObj = function (instanceType, obj, ref, options, seen) {
    ...
    if (instanceType === internals.arrayType) {
        ...
        if (!internals.isSetSimpleEqual(obj, ref)) {
            // Also check for ordered deep equality

            const objIter = obj[Symbol.iterator]();
            const refIter = ref[Symbol.iterator]();

            for (let i = 0; i < obj.size; ++i) {
                if (!isDeepEqual(objIter.next().value, refIter.next().value, options, seen)) {
                    return false;
                }
            }
        }
