...
exports.extend = function (target, source) {
    ...
    Object.keys(source).forEach(function (key) {
        ...
        else if (descriptor.value instanceof RegExp) {
            var flags = '' + (orig.global ? 'g' : '') + (orig.ignoreCase ? 'i' : '') + (orig.multiline ? 'm' : '') + (orig.sticky ? 'y' : '');
            newObj[key] = new RegExp(orig.source, flags);
        }
        else {
            // Need test cases to confirm addt'l cases, however, cursory inspection deems this currently unnecessary
        }
