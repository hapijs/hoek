...
exports.unique = function (array, keys) {
    ...
    for (var i = 0, il = array.length; i < il; ++i) {
        ...
        if (keys) {
            var key;
            for (var x = 0, xl = keys.length; x < xl; x++) {
                key = keys[x];
                value = item[key];
                id += key + typeof value + ( typeof value === 'object' ? JSON.stringify(value) : value);
            }
        } else {
            value = typeof item === 'object' ? JSON.stringify(item) : item;
            id += typeof item + value;
        }
        if (index[id] !== true) {
